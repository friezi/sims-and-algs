...
public class TextureMapping2D extends JPanel implements MouseListener, ActionListener {
	...
	private static final View2D VIEW = new View2D(screenParameters.HEIGHT, screenParameters.XNULL, screenParameters.YNULL);
	...
	private void makeImageTexture() throws IOException {
		texture = new BilinearFilter(new ImageTexture(getClass().getClassLoader().getResourceAsStream("pics/Schimpanse.jpg")));
	...
	private APointInterpolater2D makeRandomInterpolater() {
		...
		final BezierPointInterpolater interpolater = new BezierPointInterpolater(
				new Point(RANDOM.nextInt(screenParameters.RENDER_MAX_RAND_X),
		...
		for (int i = 0; i < max; i++) {
			interpolater.addControlPoint(new Point(RANDOM.nextInt(screenParameters.RENDER_MAX_RAND_X),
					screenParameters.RENDER_STARTY + RANDOM.nextInt(screenParameters.RENDER_MAX_RAND_Y)));
		}
